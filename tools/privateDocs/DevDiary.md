...
- **modifier 卡键问题已定位真正根因并修复，且已刷机实测通过**：最初怀疑是断电/断连时 `kscan_adc_mux_disable()` 不清空按键状态导致（已修复），后来发现**不拔 USB 电源时也会卡键**，定位真正主因是 Hall 轴阈值判断**完全没有消抖**，导致电压在阈值附近抖动时以扫描速率（5ms）连续产生 press/release 事件，叠加 kscan 事件队列 `k_msgq_put(K_NO_WAIT)` 满了会静默丢事件，最终让 ZMK 的 modifier 引用计数错位、卡在“按下”。已接入 ZMK 官方 `zmk_debounce` 库为 `kscan_adc_mux` 增加连续确认消抖，并给丢事件路径加上日志与更大的队列余量。**2026-07-29 刷入新编译固件后实测：长时间正常使用未再出现串键/鬼键/modifier 卡键**，阶段 6 工作内容第 7/8/9 项与对应验收标准已达成（详见 2026-07-29 日志）；
- **滚轮仍是原始 1×3 GPIO 矩阵**，A/B 两相未迁移到正交编码器，阶段 1 尚未开始，是当前短期目标里唯一还没开始动工的输入正确性问题；
- **网页上位机、OLED、电量与深度睡眠**（阶段 5、7、8）均未开始；
- **短期目标当前主线**：Hall 轴串键/鬼键/卡键三类输入正确性问题已经全部实测清零，下一步的优先级应该从“修输入 bug”切换到“补齐缺失的输入模型（正交编码器）与产品化功能（网页上位机、OLED、功耗）”，详见下方“实施原则”与阶段 1 起的最新工作重点。

### 实施原则

> **进度推进说明（2026-07-29）**：串键、鬼键、modifier 卡键这三个输入正确性问题已经全部通过实机验证修复（阶段 6 第 1/7/8/9 项），短期目标的工作重心从这里开始正式前移到**阶段 1（滚轮正交编码器）**——这是当前唯一还未开工、且会阻塞“全键位日用固件”定稿的输入模型缺陷；阶段 6 剩余的功能层设计、蓝牙 profile 键位、位置 19 冲突处理等，可以和阶段 1 并行或紧随其后推进，不必等到阶段 5/7/8 的产品化功能。
...
**进度更新（2026-07-29，实测通过）**

- 用户刷入接入 `zmk_debounce` 消抖 + 事件队列扩容后新编译的固件，**长时间正常使用未再出现串键/鬼键，也未再复现 modifier 卡键**，工作内容第 7/8/9 项与验收标准中对应的两条量化标准（长时间使用不卡键、DBG 日志下无队列丢弃）在实机上首次得到正面验证；工作内容第 10 项（连接状态变化时主动全键释放）仍未实现，但由于第 7/8/9 项已经覆盖了目前观察到的全部实际故障场景，第 10 项降级为“锦上添花的兜底”而非阻塞项，可以放到长期稳定性加固里再做。详见开发日志 2026-07-29 条目。
...
8. **（已完成，2026-07-28；2026-07-29 实机验证通过）为 `kscan_adc_mux` 接入 `zmk_debounce` 连续确认消抖**（`debounce-press-ms`/`debounce-release-ms`，默认 15 ms ≈ 3 次扫描确认），从源头抑制阈值附近的高频抖动误触发；
9. **（已完成，2026-07-28；2026-07-29 实机验证通过）给 `physical_layouts.c` 的 kscan 事件队列丢弃路径加日志**，并把 `megknob` 的 `CONFIG_ZMK_KSCAN_EVENT_QUEUE_SIZE` 从默认 4 提到 16，作为消抖之外的第二道防线；
10. **（待实现，非阻塞）评估在连接状态变化（USB 拔出、BLE 断开）时主动触发一次全键释放**，覆盖“设备未被显式 disable，但连接已经断开”的场景，不能只依赖 `kscan disable` 路径（`kscan_adc_mux_disable` 路径已覆盖，连接状态变化路径仍待实现；因第 7/8/9 项已解决目前观察到的全部卡键场景，此项优先级下调，留待长期稳定性加固阶段处理）。
...
- **（2026-07-29 已通过）设备全程保持 USB/BLE 连接、不断电不断连的情况下，长时间正常使用（含手指在按键临界行程附近停留、快速连击、多键同时按住）不出现 modifier 或普通键卡住**；
- **（新增）`CONFIG_ZMK_LOG_LEVEL` 调到 DBG 时观察不到 kscan 事件队列丢弃的 `LOG_ERR`**，作为消抖参数是否足够、队列容量是否足够的量化验收手段，尚未专门开 DBG 做过定量核对，建议后续补测留档。
...
## 2026-07-29：刷入消抖修复固件实测通过，串键/鬼键/modifier 卡键三类问题清零
...
至此，短期目标路线图（见文件顶部路线图）里三类已知的 Hall 轴输入正确性问题——**映射错位串键**（阶段 6 工作内容第 1 项，`33fdd7a2` 修复）、**阈值抖动引发的鬼键/误触发**、**事件队列丢包引发的 modifier 卡键**（阶段 6 工作内容第 7/8/9 项，`c16fcc5c` 修复）——均已完成实机验证，阶段 6 中与“输入是否正确、是否卡键”直接相关的验收标准可以标记为通过。

阶段 6 工作内容第 10 项（连接状态变化时主动全键释放）仍未实现，但目前没有实测证据表明它是当前卡键问题的必要修复项，作为非阻塞的长期稳定性加固项保留，不再拖延短期目标的推进。

`CONFIG_ZMK_LOG_LEVEL=DBG` 下核对“kscan 事件队列丢弃 `LOG_ERR` 是否完全消失”这一项量化验收标准尚未专门测过，后续如果想进一步确认消抖参数（15 ms）和队列容量（16）是否有足够裕量，可以补一次 DBG 日志观察，但不阻塞当前推进节奏。

### 路线图影响：短期目标工作重心前移

三类输入正确性问题清零后，短期目标（见文件顶部“短期目标：恢复键盘输入能力，并重新启用无线蓝牙”）的工作重心正式从“修 Hall 轴输入 bug”前移到：

1. **阶段 1：滚轮正交编码器**——当前唯一还未开工、且会阻塞“全键位日用固件”定稿的输入模型缺陷，滚轮 A/B 两相目前仍被错误建模为两个矩阵键，必须尽快切到 `alps,ec11` 正交解码；
2. **阶段 6 剩余项**：恢复全部确认存在的磁轴键、蓝牙 profile/输出切换功能层、位置 19 viewer mode 冲突处理，可以和阶段 1 并行推进；
3. 阶段 5（网页上位机）、阶段 7（OLED）、阶段 8（功耗与睡眠）仍在阶段 1/6 之后，暂不启动。
