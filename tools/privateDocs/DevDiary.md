...
- 🚧 "发送标定数值"已在网页端组装 Issue D 命令帧（`'M' 'K'` + version 3 + type=0x10 命令 + cmd=0x01 设置阈值 + len=96 + 24×(press,release)u16le + CRC-16/CCITT-FALSE，共 104 字节，CRC 已用标准向量 0x29B1 验证），连接真实设备时经 Web Serial 下发；⏳ 固件端命令解析（每键阈值数组 + 应用 + NVS）尚未实现，是接通"下发"的下一步；
